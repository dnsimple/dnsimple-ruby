...
describe Dnsimple::Client, ".zones" do
  ...
  describe "#record_distribution" do
    let(:account_id) { 1010 }
    let(:zone_id) { "example.com" }
    let(:record_id) { 5 }
    ...
    it "builds the correct request" do
      subject.record_distribution(account_id, zone_id, record_id)
    ...
    it "returns the zone record distribution check with true when the zone is fully distributed" do
      response = subject.record_distribution(account_id, zone_id, record_id)
      expect(response).to be_a(Dnsimple::Response)
    ...
    context "when the zone does not exist" do
      it "raises NotFoundError" do
        ...
        expect {
          subject.record_distribution(account_id, zone_id, "0")
        }.to raise_error(Dnsimple::NotFoundError)
      end
    end
